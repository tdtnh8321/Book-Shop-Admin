...
function ListBook(props) {
  ...
  const { mutateAsync: deleteBook } = bookQuery.deleteBook(isMutationEnabled);
    ...
    <table className="w-full border-b border-gray-200">
      <thead>
        <tr className="text-sm font-medium text-gray-700 border-b border-gray-200">
          ...
          <th className="py-4 px-4 text-center">Book</th>
        ...
        {listBook &&
          listBook.map((book, index) => (
            <tr key={index}>
              ...
              <th
                className="flex gap-x-4 items-center py-4 pl-10 cursor-pointer"
                onClick={() => {
                  setForm((prev) => ({
                    ...prev,
                    open: true,
              ...
              >
                <img
                  src={`${book.image}`}
                  class="w-40 rounded-lg object-cover object-top border border-gray-200"
                  alt="Image..."
                />
              ...
              <th className="font-medium text-center">$ {book.price}</th>
              <th className="font-medium text-center">{book.qty}</th>
              <th className="font-medium text-center">
                {book.status === 0 ? (
                  <p className="px-2 py-1 rounded-lg bg-red-500 text-white">
                    Khóa
                  </p>
                ) : book.status === 1 ? (
                  <p className="px-2 py-1 rounded-lg bg-green-500 text-white">
                    Còn hàng
                  </p>
                ) : (
      ...
      <tfoot></tfoot>
      <tr>
        <th colSpan={5} className="items-center">
          {countPage && (
            <Pagination
              numpages={countPage}
